...
import { ReactNativeNcwSdk, type IFireblocksNCWOptions, type IFireblocksNCW, type IFullKey, type IJoinWalletDescriptor, type IJoinWalletHandler, type IKeyBackupResult, type IKeyDescriptor, type IPassphraseResolver, type ITransactionSignature, type TEvent, type TMPCAlgorithm } from '.';
...
function registerEvents({ messagesHandler, secureStorageProvider, eventsHandler, ...opts }: IFireblocksNCWOptions) {    
    ...
    subscriptions.push(eventEmitter.addListener('store', async (event: MapEvent) => {
        ...
        await ReactNativeNcwSdk.handleResponse({ opId, deviceId, data: response });
    }));
    ...
    subscriptions.push(eventEmitter.addListener('contains', async (event: KeysEvent) => {
        ...
        try {
            ...
            try {
                ...
                await ReactNativeNcwSdk.handleResponse({ opId, deviceId, data: Object.fromEntries(keys.map(k => [k, allKeys.has(k)])) });
            } finally {
                await release();
            }
        } catch (e) {
    ...
    subscriptions.push(eventEmitter.addListener('outgoingMessage', async (event) => {
        ...
        try {
            ...
            // Handle response that might be wrapped in "response" object
            const actualResponse = response && (response as any).response 
                ? (response as any).response 
                : response;

            await ReactNativeNcwSdk.handleResponse({ opId, deviceId, data: JSON.stringify(actualResponse) });
        } catch (e) {
...
export async function FireblocksNCWFactory(options: IFireblocksNCWOptions): Promise<IFireblocksNCW> {
    ...
    const bridge: IFireblocksNCW = {
        ...
        generateMPCKeys: async function (algorithms: Set<TMPCAlgorithm>): Promise<Set<IKeyDescriptor>> {
            return new Set(await ReactNativeNcwSdk.generateMPCKeys(deviceId, Array.from(algorithms)));
        },
        ...
        backupKeys: async function (passphrase: string, passphraseId: string): Promise<IKeyBackupResult[]> {
            return await ReactNativeNcwSdk.backupKeys(deviceId, passphrase, passphraseId);
        },
        ...
        requestJoinExistingWallet: async function (joinWalletResolver: IJoinWalletHandler): Promise<Set<IKeyDescriptor>> {
            ...
            try {
                ...
                return new Set(await ReactNativeNcwSdk.requestJoinExistingWallet(deviceId));
            }
            catch (e) {
        ...
        approveJoinWalletRequest: async function (requestId: string): Promise<Set<IJoinWalletDescriptor>> {
            return new Set(await ReactNativeNcwSdk.approveJoinWalletRequest(deviceId, requestId));
        },
        ...
        deriveAssetKey: function (extendedPrivateKey: string, coinType: number, account: number, change: number, index: number): string {
            const key = ReactNativeNcwSdk.deriveAssetKey(deviceId, extendedPrivateKey, { coinType, account, change, index });
            return key;
        },
