...
class DeviceAdapter : KeyStorageDelegate, MessageHandlerDelegate, EventHandlerDelegate, FireblocksPassphraseResolver, FireblocksJoinWalletHandler {
    ...
    private func sendOperation(eventName: String, params: [ String : Any ], responseHandler: @escaping (NSDictionary) -> ()) {
        ...
        NcwSdk.emitter.sendEvent(withName: eventName, body: reqBody)
    }
    
    // FireblocksJoinWalletHandler
    func onRequestId(requestId: String) {
        NcwSdk.emitter.sendEvent(withName: "joinWalletRequest", body: [
            "deviceId": deviceId,
            "requestId": requestId ])
    }
    ...
    func onProvisionerFound() {
        NcwSdk.emitter.sendEvent(withName: "provisionerFound", body: [
            "deviceId": deviceId ])
    }
    ...
    func onEvent(event: FireblocksSDK.FireblocksEvent) {
        func emitSDKEvent(body: [String: Any]) {
            NcwSdk.emitter.sendEvent(withName: "sdk_event", body: body)
        }
        do {
            switch event {
    ...
    func contains(keyIds: Set<String>, callback: @escaping ([String : Bool]) -> ()) {
        ...
        NcwSdk.emitter.sendEvent(withName: "contains", body: [])
    }
...
class NcwSdk : RCTEventEmitter {
    ...
    func handleResponse(_ response: NSDictionary,
        ...
        if NcwSdk.adapters.contains(where: { $0.key == deviceId }) {
            NcwSdk.adapters[deviceId]?.handleResponse(response: response)
        }
    ...
    func initialize(_ deviceId: String, withResolve resolve: RCTPromiseResolveBlock, withReject reject: RCTPromiseRejectBlock) {
        do {
            ...
            NcwSdk.adapters[deviceId] = adapter
            resolve(nil)
        } catch let err {
    ...
    @objc func recoverKeys(_ deviceId: String, withResolve resolve: @escaping RCTPromiseResolveBlock, withReject reject: @escaping RCTPromiseRejectBlock) {
        do {
            let instance = try Fireblocks.getInstance(deviceId: deviceId)
            let adapter = NcwSdk.adapters[deviceId]!
