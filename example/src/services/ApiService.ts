...
export class ApiService {
  ...
  private async _postCall(path: string, body?: Object): Promise<any> {
    ...
    try {
      ...
      console.log(`Making POST request to: ${this._baseUrl}/${path}`);
      ...
      if (!response.ok) {
        ...
        console.error(
          `API Error for ${path}: ${response.status} - ${errorText}`
        );
        throw new Error(
          `A call to "${path}" failed with status ${response.status}: ${errorText}`
        );
      }
