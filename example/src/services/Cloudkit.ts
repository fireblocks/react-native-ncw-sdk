...
export const cloudkitBackup = async (cloudkit: any, passphrase: string, passphraseId: string) => {
...
export const cloudkitRecover = async (cloudkit: any, passphraseId: string) => {
  const db = cloudkit.getDefaultContainer().privateCloudDatabase;
  const recordName = `backup_t_${passphraseId}`;
  const recordType = 'Backup';
