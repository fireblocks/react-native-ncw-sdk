...
import { NcwSdk } from "react-native-ncw-sdk";
...
export async function FireblocksNCWFactory(options) {
  const {
    deviceId
  } = options;
  ...
  const bridge = {
    ...
    deriveAssetKey: function (extendedPrivateKey, coinType, account, change, index) {
      const key = NcwSdk.deriveAssetKey(deviceId, extendedPrivateKey, {
        coinType,
        account,
        change,
        index
